...
$(document).ready(function () {
  ...
  $("#login-ati").submit(function (e) {
    e.preventDefault();
    start_loader();
    if ($(".err_msg").length > 0) $(".err_msg").remove();
    $.ajax({
      url: _base_url_ + "classes/Login.php?f=login_ati",
      method: "POST",
      data: $(this).serialize(),
      error: (err) => {
        console.log(err);
      ...
      success: function (resp) {
        ...
        if (resp) {
          ...
          console.log(resp); // Log the parsed response
          if (resp.status == "success") {
            location.replace(_base_url_ + "ati");
          } else if (resp.status == "incorrect") {
            var _frm = $("#login-ati");
            var _msg =
              "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Incorrect username or password</div>";
            ...
            var _frm = $("#login-ati");
            var _msg =
              "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Access Denied! ATI Role</div>";
            _frm.prepend(_msg);
            _frm.find("input").addClass("is-invalid");
            $('[name="username"]').focus();
          }
  ...
  $("#login-bpi").submit(function (e) {
    e.preventDefault();
    start_loader();
    if ($(".err_msg").length > 0) $(".err_msg").remove();
    $.ajax({
      url: _base_url_ + "classes/Login.php?f=login_bpi",
      method: "POST",
      data: $(this).serialize(),
      error: (err) => {
        console.log(err);
      ...
      success: function (resp) {
        ...
        if (resp) {
          ...
          console.log(resp); // Log the parsed response
          if (resp.status == "success") {
            location.replace(_base_url_ + "bpi");
          } else if (resp.status == "incorrect") {
            var _frm = $("#login-bpi");
            var _msg =
              "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Incorrect username or password</div>";
            ...
            var _frm = $("#login-bpi");
            var _msg =
              "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Access Denied! BPI Role</div>";
            _frm.prepend(_msg);
            _frm.find("input").addClass("is-invalid");
            $('[name="username"]').focus();
          }
